...
**Design pattern (2026-08-09):** dark full-bleed motion (not white-background). Built for **B&M Pressure Washing** and any pressure-washing / soft-wash / exterior-cleaning prospect.
...
1. `mkdir demos/<slug> && cp -r templates/pressure-washing/* demos/<slug>/`
2. Fill the 15 `{{PLACEHOLDERS}}` in `index.html`:
   - `{{BUSINESS_NAME}}`, `{{CITY}}`, `{{STATE_ABBR}}`, `{{REGION}}`
   - `{{PHONE}}`, `{{PHONE_LINK}}`
   - `{{HERO_HEADLINE_1}}` / `{{HERO_HEADLINE_2}}` (e.g. "Clean Driveways." / "Fresh & Curb-Ready.")
   - `{{OWNERSHIP}}`, `{{LOCAL_TRUST}}`, `{{ABOUT_OWNER}}`
   - `{{REVIEW_NAME}}`, `{{REVIEW_LOCATION}}`, `{{REVIEW_SOURCE}}`
   - `{{LOGO_IMG}}` (point at the prospect's real logo asset)
3. Swap the real before/after photos (`driveway-before-real.jpg` / `driveway-after-real.jpg`).
4. Fill `{{SLUG}}` in the deploy sample URL (clean-driveway.py `VISUALIZER_SAMPLE`).
5. Set env vars + deploy via `npx vercel deploy --prod --yes`.
