#!/usr/bin/env python3
"""Deploy palmetto-landscaping demo to Vercel as standalone project."""
import json, hashlib, os, sys, time, requests

# ─── REDUNDANCY GUARDRAIL (auto-run every deploy) ────────────────────────────
# Fail the deploy before uploading if any image repeats across sections or a
# referenced file is missing. See /mnt/d/LocalLaunch/tools/check_redundancy.py
def _run_guardrail():
    tool = "/mnt/d/LocalLaunch/tools/check_redundancy.py"
    site = os.path.dirname(os.path.abspath(__file__))
    if os.path.exists(tool):
        code = os.system(f"{sys.executable} {tool} {site}")
        if code != 0:
            sys.exit(f"\n❌ REDUNDANCY GUARDRAIL FAILED for {site}.\n"
                     "Fix the duplicate/missing images (each section needs DISTINCT "
                     "images) before this demo can ship. See "
                     "/mnt/d/LocalLaunch/tools/check_redundancy.py output above.")
_run_guardrail()
# ──────────────────────────────────────────────────────────────────────────────
...
PROJECT = "steve-cantrell-tree-demo"
