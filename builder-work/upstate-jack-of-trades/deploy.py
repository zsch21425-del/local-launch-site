#!/usr/bin/env python3
"""Deploy upstate-jack-of-trades demo to Vercel as standalone project."""
...
PROJECT = "upstate-jack-of-trades-demo"
...
def compute_files():
    ...
    for root, dirs, filenames in os.walk(SITE_DIR):
        dirs[:] = [d for d in dirs if not d.startswith(".git") and not d.startswith("__") and not d.startswith(".")]
        for fn in filenames:
            if fn in ("deploy.py", "TEMPLATE-BRIEF.md", "BRIEF.md", "requirements.txt"):
                continue
