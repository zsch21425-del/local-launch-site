...
const TYPE_META: Record<string, { icon: string; color: string }> = {
  ...
  playbook: { icon: "✅", color: "text-primary" },
  pitch: { icon: "📨", color: "text-sky-500" },
  audit: { icon: "🔍", color: "text-violet-500" },
  contact: { icon: "📞", color: "text-muted-foreground" },
  ...
  return (
    <div className={cn(glass, "rounded-xl p-5")}>
      <div className="flex items-center justify-between mb-4">
        ...
        <button onClick={load} className="text-xs text-muted-foreground transition-colors hover:text-foreground" aria-label="Refresh activity">
          <RefreshCw className={cn("size-3.5", loading && "animate-spin")} />
        </button>
      </div>

      {error === "local-only" ? (
        <p className="text-sm text-muted-foreground py-2">Activity from the vault is available on the local dashboard only.</p>
      ) : error ? (
      ...
      {loading && events.length === 0 ? (
        ...
        <p className="text-sm text-muted-foreground py-2">No recorded activity yet for this client.</p>
      ) : (
        <ol className="relative flex flex-col gap-3 before:absolute before:left-[5px] before:top-1 before:bottom-1 before:w-px before:bg-muted">
          {events.map((e, i) => {
            const meta = TYPE_META[e.type] ?? TYPE_META.contact;
            return (
              <li key={i} className="relative pl-6">
                <span className="absolute left-0 top-0.5 grid size-[11px] place-items-center rounded-full bg-card ring-1 ring-border text-[8px]">
                  <span className={cn("text-[9px] leading-none", meta.color)}>{meta.icon}</span>
                </span>
                <div className="flex items-baseline gap-2">
                  <span className="text-[11px] font-medium text-muted-foreground">{fmtDate(e.ts)}</span>
                  {e.actor && <span className="text-[11px] text-muted-foreground">{e.actor}</span>}
                </div>
                <p className="text-sm text-muted-foreground leading-snug">{e.detail}</p>
              </li>
            );
          })}
        </ol>
